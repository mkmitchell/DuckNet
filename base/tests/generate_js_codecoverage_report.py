...
for js in jsonfiles:
    ...
    jsondata = dict(
        [
            ('/'.join(x['url'].split('/')[-2:]), x)
            for x in jsondata
            if 'thirdparty' not in x['url']
        ]
    )
...
def merge_ranges(*list_of_ranges):
    ...
    merged_starts = [0] + [
        i + 1
        for i, x in enumerate(covered_chars[:-1])
        if x == 0 and covered_chars[i + 1] == 1
    ]
    merged_ends = [
        i + 1
        for i, x in enumerate(covered_chars[:-1])
        if x == 1 and covered_chars[i + 1] == 0
    ]
...
for url, data_list in jsondata_per_url.items():
    list_of_ranges = [
        [(r['start'], r['end']) for r in d['ranges']] for d in data_list
    ]
...
for url, ranges in merged_ranges.items():
    ranges_flat = [0] + [i for pair in ranges for i in pair]
    splits = [
        texts_per_url[url][i:j] for i, j in zip(ranges_flat, ranges_flat[1:])
    ]
    splits = [
        f'<span style="background:pink;">{s}</span>' if i % 2 == 0 else s
    ...
    open(
        os.path.expanduser(f'{OUTPUT_DIR}/{url.replace("/", "_")}.html'), 'w'
    ).write(html)

    prcnt = (
        sum([end - start for start, end in ranges])
        / len(texts_per_url[url])
        * 100
    )
