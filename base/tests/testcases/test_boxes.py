...
class TestBoxes(BaseCase):
    ...
    def test_add_boxes(self):
        ...
        webdriver.ActionChains(self.driver).click_and_hold(img).move_by_offset(
            100, 100
        ).release().perform()
        self.sleep(0.1)
        ...
        assert boxes1[0][0] > boxes0[0][0] and boxes1[0][1] < boxes0[0][1], (
            boxes0,
            boxes1,
        )
        # assert the box width/height has not changed
        assert (
            abs((boxes1[0][0] - boxes1[0][2]) - (boxes0[0][0] - boxes0[0][2]))
            < 1
            and abs(
                (boxes1[0][1] - boxes1[0][3]) - (boxes0[0][1] - boxes0[0][3])
            )
            < 1
        ), (boxes0, boxes1)
