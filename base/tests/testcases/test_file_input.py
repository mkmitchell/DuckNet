...
class TestBasic(BaseCase):
    def test_open_and_display(self):
        ...
        self.highlight(
            'th:contains(" Files Loaded")'
        )  # FIXME: 2 files in chrome, 3 files in firefox (webdriver issue)

        # open one file
        self.click('label:contains("test_image1.jpg")')
        # wait for the image to display
        self.wait_for_element_not_visible(
            '[filename="test_image1.jpg"]:contains("Loading")', timeout=6
        )
    ...
    def test_load_tiff(self):
        ...
        # wait for the image to display
        self.wait_for_element_not_visible(
            '[filename="test_image2.tiff"]:contains("Loading")', timeout=6
        )
        assert self.is_element_visible('[filename="test_image2.tiff"] img')

        # a screenshot of the tiff row can be saved here when debugging
    ...
    def test_load_results(self):
        ...
        script = (
            ''' return $('[filename="test_image1.jpg"] label')'''
            '''.css('font-weight') '''
        )
        assert int(self.execute_script(script)) > 500
        # image0 should be not loaded
        script = (
            ''' return $('[filename="test_image0.jpg"] label')'''
            '''.css('font-weight') '''
        )
        assert int(self.execute_script(script)) <= 500

        # load the result of the other image
        self.send_input_files_from_assets(["test_image0.jpg.results.zip"])
        self.sleep(0.5)
...
@pytest.mark.parametrize(
    'imagesize',
    [(256, 256), (4096, 4096), (256, 4096), (4096, 256), (16, 64), (64, 16)],
)  # order: width-height
def test_aspect_ratios(imagesize):
    class AspectRatios(BaseCase):
        def perform_test(self, imagesize):
            ...
            data = (np.random.random(imagesize[::-1] + (3,)) * 255).astype(
                'uint8'
            )
            ...
            self.save_screenshot(
                screenshot,
                selector='[filename="test_image.jpg"] img.input-image',
            )
