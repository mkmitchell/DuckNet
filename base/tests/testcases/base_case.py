...
from seleniumbase import BaseCase as SeleniumBaseCase
...
class BaseCase(SeleniumBaseCase):
    ...
    def tearDown(self):
        if self.is_chromium():
            ...
            outputfile = os.path.join(
                self.log_abspath,
                'coverage_js/raw',
                f'{self.test_id}.codecoverage.json',
            )
    ...
    def open_main(self, static=True):
        if static:
            return self.open(
                f"file://{os.environ['INSTANCE_PATH']}/static/index.html"
            )
        else:
    ...
    def start_flask(self):
        ...
        t = threading.Thread(
            target=lambda: App().run(debug=False, port=port, parse_args=False),
            daemon=True,
        )
        t.start()
        return port

    def send_input_files_from_assets(
        self, files: list, input_id='input_images'
    ):
        ...
        paths = [
            (
                glob.glob(os.path.join(asset_dirs[0], f))
                + glob.glob(os.path.join(asset_dirs[1], f))
            )[0]
            for f in files
        ]
