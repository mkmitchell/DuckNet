...
class ProcessingTest(BaseCase):
    ...
    def test_basic_processing_success(self):
        ...
        # a dimmer indicating that processing is in progress should be visible
        ...
        self.execute_script(
            f'''$('{root_css} .dimmer').click()'''
        )  # self.click doesnt work
        self.sleep(0.5)
        ...
        # the row label must be bold to indicate that this file is processed
        script = f''' return $('{root_css} label').css('font-weight') '''
        assert int(self.execute_script(script)) > 500

        # the result image starts grayed out with contrast(0); not any more
        ...
        # TODO: check the finished indication (bold font in the file list)
    ...
    def test_basic_upload(self):
        ...
        script = (
            '''upload_file_to_flask(GLOBAL.files["test_image1.jpg"])'''
            '''.done(arguments[0]())'''
        )
        self.execute_async_script(script)
        self.sleep(1)
        self.assert_link_status_code_is_not_404(
            f"http://localhost:{self.port}/images/test_image1.jpg"
        )
    ...
    def test_upload_failed(self):
        ...
        assert (
            self.execute_script(
                '''return $('.dimmer :contains("failed"):visible').length'''
            )
            > 0
        )
        # wait_for_element_visible on the failed dimmer does not work here

        # dimmer should hide after click
        self.execute_script(
            '''$('.dimmer:visible').click()'''
        )  # self.click doesnt work
        self.wait_for_element_not_visible(root_css + ' .dimmer', timeout=6)
