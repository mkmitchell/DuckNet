...
class TestViewControls(BaseCase):
    ...
    def test_brightness(self):
        ...
        self.send_input_files_from_assets(
            ["test_image0.jpg", "test_image1.jpg"]
        )
        ...
        brightness0 = float(
            image_filters.split('brightness(')[1].split(')')[0]
        )
        ...
        assert (
            abs(brightness2 - brightness0) < 0.12
    ...
    def test_panning(self):
        ...
        script = (
            f''' return $('{root_css} .transform-box').css('transform') '''
        )
        xform_matrix0_str = self.execute_script(script)
        assert 'matrix(1, 0, 0, 1, 0, 0)' == xform_matrix0_str
        xform_matrix0 = self.execute_script(
            f'return new DOMMatrix("{xform_matrix0_str}")'
        )
        ...
        webdriver.ActionChains(self.driver).key_down(
            Keys.SHIFT
        ).click_and_hold(img).move_by_offset(20, 0).release().perform()
        self.sleep(0.1)

        # transform matrix should have changed
        script = (
            f''' return new DOMMatrix( $('{root_css} .transform-box')'''
            '''.css('transform') ) '''
        )
        ...
        # no text may be selected by the mouse dragging (a real small bug)
        assert self.execute_script('return getSelection().type') != 'Range'

        # move again, without shift key
        webdriver.ActionChains(self.driver).key_up(Keys.SHIFT).click_and_hold(
            img
        ).move_by_offset(0, 20).release().perform()
        self.sleep(0.1)
        ...
        # zoom
        script_zoom = (
            f''' $('{root_css} .transform-box')[0].dispatchEvent('''
            ''' new WheelEvent("wheel", {deltaY:-10, shiftKey:true}) ) '''
        )
        ...
        assert all(
            [xform_matrix1[k] == xform_matrix3[k] for k in 'bcf']
        )  # e also changes because panned

        # double-click with shift key
        webdriver.ActionChains(self.driver).key_down(Keys.SHIFT).double_click(
            img
        ).perform()
        self.sleep(0.1)
