...
class SettingsTests(BaseCase):
    def test_save_settings_failed(self):
        ...
        # the 'loading' state is visible only very briefly; not testable here
        ...
        elements = self.find_visible_elements(
            '.error:contains("Saving failed")'
        )
        assert len(elements) > 0
        # remove the loading class
        assert 'loading' not in self.get_attribute(
            "div#settings-ok-button", 'class'
        )
    ...
    def test_save_settings_keep_values(self):
        ...
        for idx in [0, -1]:
            ...
            css_active_model = (
                '[id^="settings-active-"][id$="-model"]'  # XXX: hacky
            )
            self.click(css_active_model)
            # the dropdown list has items and they are now visible
            self.wait_for_element_visible(
                f'{css_active_model} .item', timeout=0.5
            )
            ...
            # click on settings to open dialog
            self.click("#settings-button")
            selected = self.get_attribute(
                f'{css_active_model} .text', 'innerText'
            )
