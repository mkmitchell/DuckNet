...
class TrainingTask(torch.nn.Module):
    ...
    def warmup_lr_scheduler(self, optimizer, num_batches_in_epoch):
        """Scheduler warming up linearly from 10% to 100% over epoch 0."""
    ...
    def fit(self, loader_train, loader_valid=None, epochs='auto'):
        ...
        if self.progress_callback is not None:
            self.callback = TrainingProgressCallback(
                self.progress_callback, self.epochs
            )
        else:
        ...
        try:
            ...
            for e in range(self.epochs):
                ...
                epoch_terminated = self.train_one_epoch(
                    loader_train, optim, sched
                )
                if epoch_terminated:
                ...
                if self.is_stop_requested():
                    print(
                        f"Training stopped after epoch {e} before validation"
                    )
                    break
...
class DetectionTask(TrainingTask):
    def training_step(self, batch):
        ...
        y = [
            dict([(k, yy[k].to(self.device)) for k in ['boxes', 'labels']])
            for yy in y
        ]
        lossdict = self.basemodule(x, y)
        loss = torch.stack([*lossdict.values()]).sum()
        logs = dict(
            [(k, v.item()) for k, v in lossdict.items()]
            + [('loss', loss.item())]
        )
        return loss, logs
    ...
    def validation_epoch_end(self, logs):
        def gather(split, key):
            return [item[key].cpu() for batch in logs for item in batch[split]]

        boxes_true = gather('y_true', 'boxes')
        boxes_pred = gather('y_pred', 'boxes')
        scores_pred = gather('y_pred', 'scores')
        labels_true = gather('y_true', 'labels')
        labels_pred = gather('y_pred', 'labels')
        ...
        for class_idx, class_name in enumerate(class_list):
            ...
            for i, (b_pred, l_pred, s_pred) in enumerate(
                zip(boxes_pred, labels_pred, scores_pred)
            ):
                mask = l_pred == class_idx_numeric
                class_boxes_pred.append(b_pred[mask])
                class_scores_pred.append(s_pred[mask])

            if len(class_boxes_true) > 0 and any(
                len(b) > 0 for b in class_boxes_true
            ):
                try:
                    class_metrics_cache = precompute_metrics(
                        class_boxes_true, class_boxes_pred, class_scores_pred
                    )

                    ap_50 = cached_average_precision(
                        class_metrics_cache, iou_threshold=0.5
                    )
                    ar_50 = cached_max_recall(
                        class_metrics_cache, iou_threshold=0.5
                    )
                    f1_50 = calculate_f1_score(ap_50, ar_50)

                    ap_75 = cached_average_precision(
                        class_metrics_cache, iou_threshold=0.75
                    )
                    ar_75 = cached_max_recall(
                        class_metrics_cache, iou_threshold=0.75
                    )
                    f1_75 = calculate_f1_score(ap_75, ar_75)

                    ap_range = np.mean(
                        [
                            cached_average_precision(
                                class_metrics_cache, float(iou)
                            )
                    ...
                    ar_range = np.mean(
                        [
                            cached_max_recall(class_metrics_cache, float(iou))
                            for iou in iou_range
                        ]
                    )
                    f1_range = calculate_f1_score(ap_range, ar_range)
                    ...
                    per_class_metrics[class_name] = {
                        ...
                        'AR@IoU=0.75': ar_75,
                        'F1@IoU=0.75': f1_75,
                        'AP@IoU=0.5:0.95': ap_range,
                        'AR@IoU=0.5:0.95': ar_range,
                        'F1@IoU=0.5:0.95': f1_range,
                    }

                    metrics[f'{class_name}_AP@IoU=0.5'] = ap_50
                    metrics[f'{class_name}_AP@IoU=0.75'] = ap_75
                    metrics[f'{class_name}_AP@IoU=0.5:0.95'] = ap_range

                except Exception as e:
                    print(
                        "Error calculating metrics for class "
                        f"{class_name}: {e}"
                    )
                    continue

        if per_class_metrics:
            metrics['AP@IoU=0.5'] = np.mean(
                [m['AP@IoU=0.5'] for m in per_class_metrics.values()]
            )
            metrics['AR@IoU=0.5'] = np.mean(
                [m['AR@IoU=0.5'] for m in per_class_metrics.values()]
            )
            metrics['F1@IoU=0.5'] = np.mean(
                [m['F1@IoU=0.5'] for m in per_class_metrics.values()]
            )

            metrics['AP@IoU=0.75'] = np.mean(
                [m['AP@IoU=0.75'] for m in per_class_metrics.values()]
            )
            metrics['AR@IoU=0.75'] = np.mean(
                [m['AR@IoU=0.75'] for m in per_class_metrics.values()]
            )
            metrics['F1@IoU=0.75'] = np.mean(
                [m['F1@IoU=0.75'] for m in per_class_metrics.values()]
            )

            metrics['AP@IoU=0.5:0.95'] = np.mean(
                [m['AP@IoU=0.5:0.95'] for m in per_class_metrics.values()]
            )
            metrics['AR@IoU=0.5:0.95'] = np.mean(
                [m['AR@IoU=0.5:0.95'] for m in per_class_metrics.values()]
            )
            metrics['F1@IoU=0.5:0.95'] = np.mean(
                [m['F1@IoU=0.5:0.95'] for m in per_class_metrics.values()]
            )
...
def precompute_metrics(boxes_true, boxes_pred, scores_pred, n_thresholds=51):
    """
    ...
    """
    cache = {}
    confidence_thresholds = np.linspace(0, 1, n_thresholds)

    for i, (b_true, b_pred, s_pred) in enumerate(
        zip(boxes_true, boxes_pred, scores_pred)
    ):
        ...
        iou_matrix = (
            torchvision.ops.box_iou(
                torch.as_tensor(b_true), torch.as_tensor(b_pred)
            )
            .cpu()
            .numpy()
        )
        ...
        for threshold in confidence_thresholds:
            ...
            if filtered_matrix.size > 0:
                ixs0, ixs1 = scipy.optimize.linear_sum_assignment(
                    filtered_matrix, maximize=True
                )
                matches = [
                    (i, j, filtered_matrix[i, j]) for i, j in zip(ixs0, ixs1)
                ]
            else:
                matches = []
...
def cached_precision_recall(metrics_cache, iou_threshold=0.5):
    """
    Precision and recall from cached matches at the given IoU threshold
    """
    n_thresholds = 51  # Number of confidence thresholds
    confidence_thresholds = np.linspace(0, 1, n_thresholds)
    ...
    for i, threshold in enumerate(confidence_thresholds):
        for batch_cache in metrics_cache.values():
            ...
            valid_matches = sum(
                1
                for _, _, iou in cache_entry['matches']
    ...
    def on_epoch_end(self, epoch):
        ...
        if 'per_class' in self.logs:
            ...
            for class_name, metrics in per_class.items():
                ...
                for iou in ('0.5', '0.75'):
                    print(
                        f"  AP@IoU={iou}: {metrics[f'AP@IoU={iou}']:.4f}, "
                        f"AR@IoU={iou}: {metrics[f'AR@IoU={iou}']:.4f}, "
                        f"F1@IoU={iou}: {metrics[f'F1@IoU={iou}']:.4f}"
                    )
...
class TrainingProgressCallback:
    ...
    def on_batch_end(self, logs, batch_i, n_batches):
        ...
        percent = (self.epoch * n_batches + (batch_i + 1)) / (
