...
```{r human_pheno_plot}
df2=read.csv("./untrained_data/human/Batzbach_pseudo_corrected.csv")
head(df2)
...
Species_list=c("Mnat","Mdau","Mmyo","Paur")

  for (Species in Species_list)   {
    Species_plot=ifelse(Species=="Mnat","Myotis nattereri",
                        ifelse(Species=="Mdau","Myotis daubentonii",
                          ifelse(Species=="Paur","Plecotus auritus","Myotis myotis")))
   ...
   df2 %>% 
    ...
    ggplot(aes(x=pseudodate, y=n))+
      ...
      scale_x_date(breaks = monthly, date_labels= "%b %d", 
                         limits=c((as.Date("2021-08-01")),as.Date("2021-12-31")))+
      theme(axis.text.x = element_text(angle=45, hjust = 1))+
     labs(title=bquote('Batzbach -'~italic(.(Species_plot))),
                      x="", 
      ...
      theme_bw()+
      geom_label(x = as.Date("2021-12-15"), y = max*0.8, 
            aes(label = paste0("N=",scales::comma(N))),  
            inherit.aes = FALSE, fill="white")+
     theme(plot.title = element_text(hjust = 0.5))+
    scale_y_continuous(breaks=pretty_breaks(n=6))
   
   ggsave(paste0("./real_life_data/phenology/Batzbach_human_",Species,"_corrected.png"),
       width =15, height =7, dpi = 800, units = "cm", device='png')
  }



write.csv(percentiles_hum[-1,], paste0("./real_life_data/phenology/batz_hum.csv"), 
             row.names = F,
             quote = F)
hum=df2 %>% 
...
```{r AI_pheno_plot}
df2=read.csv("./untrained_data/AI/Batzbach_pseudo.csv") #AI ID with pseudodays
head(df2)
...
Species_list=c("Mnat","Mdau","Mmyo","Paur")
  ...
  for (cSpecies in Species_list)   {
    ...
    Species_plot=ifelse(cSpecies=="Mnat","Myotis nattereri",
                        ifelse(cSpecies=="Mdau","Myotis daubentonii",
                          ifelse(cSpecies=="Paur","Plecotus auritus","Myotis myotis")))
   ...
   df2 %>% 
    ...
    scale_y_continuous(breaks=pretty_breaks(n=6))
      ...
      ggsave(paste0("./real_life_data/phenology/Batzbach_th70_",cSpecies,".png"),
       width =15, height =7, dpi = 800, units = "cm", device='png')
  }

write.csv(percentiles_alg[-1,], paste0("./real_life_data/phenology/batz_alg.csv"), 
             row.names = F,
             quote = F)
AI=df2 %>% 
  filter(Confidence.level>=70) %>% 
...
for(cSpec in Species_list){
   Species_plot=ifelse(cSpec=="Mnat","Myotis nattereri",
                        ifelse(cSpec=="Mdau","Myotis daubentonii",
                               ifelse(cSpec=="Paur","Plecotus auritus","Myotis myotis")))
...
mdf %>% 
  ...
  ggsave(paste0("./real_life_data/concordance/Batzbach_",cSpec,"_corrected.png"),
...
```{r mismatches}
full=read.csv("./real_life_data/mismatch_checking/Batzbach_corrected.csv")
...
levels(above_th$actual)=c("Empty","Myotis bechsteinii","Myotis brandtii", "Myotis daubentonii","Myotis myotis","Myotis nattereri","Plecotus sp.")

above_th$pred=as.factor(above_th$pred)
levels(above_th$pred)
levels(above_th$pred)=c("Barbastella barbastellus","Empty","Myotis bechsteinii","Myotis brandtii", "Myotis dasycneme", "Myotis daubentonii","Myotis emarginatus","Myotis myotis","Myotis nattereri","Plecotus sp.","Pipistrellus sp.","Rhinolophus sp.")
...
ggsave(paste0("./real_life_data/Batzbach_70_cm_corrected.png"),
...
empty_lost=full %>% 
  ...
  count() %>% 
  filter(Species.x=="Empty")
lost=rbind(as.data.frame(lost),c(Species.x="Bbar",n=""))  
lost=rbind(as.data.frame(lost),c(Species.x="Mdas",n=""))  
lost=rbind(as.data.frame(lost),c(Species.x="Mema",n="")) 
lost=rbind(as.data.frame(lost),c(Species.x="Ppip",n="")) 
lost=rbind(as.data.frame(lost),c(Species.x="Rhin",n="")) 
lost =
  lost %>% 
  arrange(Species.x)
...
multiple=rbind(as.data.frame(multiple),c(Species.x="Mema",n="")) 
multiple=rbind(as.data.frame(multiple),c(Species.x="Ppip",n="")) 
multiple =
  multiple %>% 
  arrange(Species.y)
...
cm2 %>% 
  ...
  annotate(geom="rect", xmin=0.5, xmax=1.5, ymin=12.5, ymax=13.5,fill="white",alpha=0.6)+ #0
  annotate(geom="rect",xmin=1.5, xmax=2.5, ymin=12.5, ymax=13.5,fill="#C1BBDD",alpha=0.6)+ #222
  annotate(geom="rect",xmin=2.5, xmax=3.5, ymin=12.5, ymax=13.5,fill="#C1BBDD",alpha=0.6)+ #128
  annotate(geom="rect",xmin=3.5, xmax=4.5, ymin=12.5, ymax=13.5,fill="white",alpha=0.6)+ #0
  annotate(geom="rect",xmin=4.5, xmax=5.5, ymin=12.5, ymax=13.5,fill="#C1BBDD",alpha=0.6)+ #200
  annotate(geom="rect",xmin=5.5, xmax=6.5, ymin=12.5, ymax=13.5,fill="white",alpha=0.6)+ #0
  annotate(geom="rect",xmin=6.5, xmax=7.5, ymin=12.5, ymax=13.5,fill="lightblue1",alpha=0.6)+ #40
  annotate(geom="rect",xmin=7.5, xmax=8.5, ymin=12.5, ymax=13.5,fill="mediumpurple",alpha=0.6)+ #1433
  annotate(geom="rect",xmin=8.5, xmax=9.5, ymin=12.5, ymax=13.5,fill="#F5FFFF",alpha=0.6)+ #8
    annotate(geom="rect",xmin=9.5, xmax=10.5, ymin=12.5, ymax=13.5,fill="white",alpha=0.6)+ #0
    annotate(geom="rect",xmin=10.5, xmax=11.5, ymin=12.5, ymax=13.5,fill="white",alpha=0.6)+ #0
    annotate(geom="rect",xmin=11.5, xmax=12.5, ymin=12.5, ymax=13.5,fill="#F5FFFF",alpha=0.6)+ #8
  
  #multiple bat values
  annotate(geom="rect",xmin=12.5, xmax=13.5, ymin=0.5, ymax=1.5,fill="#F5FFFF",alpha=0.6)+ #3
  annotate(geom="rect",xmin=12.5, xmax=13.5, ymin=1.5, ymax=2.5,fill="#C1BBDD",alpha=0.6)+ #243
  annotate(geom="rect",xmin=12.5, xmax=13.5, ymin=2.5, ymax=3.5,fill="lightblue1",alpha=0.6)+ #27
  annotate(geom="rect",xmin=12.5, xmax=13.5, ymin=3.5, ymax=4.5,fill="#F5FFFF",alpha=0.6)+ #7
  annotate(geom="rect",xmin=12.5, xmax=13.5, ymin=4.5, ymax=5.5,fill="lightblue1",alpha=0.6)+ #56
  annotate(geom="rect",xmin=12.5, xmax=13.5, ymin=5.5, ymax=6.5,fill="white",alpha=0.6)+ #0
  annotate(geom="rect",xmin=12.5, xmax=13.5, ymin=6.5, ymax=7.5,fill="lightblue1",alpha=0.6)+ #25
  annotate(geom="rect",xmin=12.5, xmax=13.5, ymin=7.5, ymax=8.5,fill="#C1BBDD",alpha=0.6)+ #429
  annotate(geom="rect",xmin=12.5, xmax=13.5, ymin=8.5, ymax=9.5,fill="lightblue1",alpha=0.6)+ #34
  annotate(geom="rect",xmin=12.5, xmax=13.5, ymin=9.5, ymax=10.5,fill="white",alpha=0.6)+ #0
  annotate(geom="rect",xmin=12.5, xmax=13.5, ymin=10.5, ymax=11.5,fill="#F5FFFF",alpha=0.6)+ #1
   annotate(geom="rect",xmin=12.5, xmax=13.5, ymin=11.5, ymax=12.5,fill="white",alpha=0.6)+ #0
  ...
  annotate("text", x = length(all_levels)+2, y = length(all_levels)+1, label ="F1-score",size=2.2,angle=45,vjust=0.5,hjust=0.4)+
  annotate("text", x = length(all_levels)+3, y = length(all_levels)+1, label ="Precision",size=2.2,angle=45,vjust=0.5,hjust=0.4)+
  annotate("text", x = length(all_levels)+4, y = length(all_levels)+1, label ="Recall",size=2.2,angle=45,vjust=0.5,hjust=0.4)+
  ...
  annotate("text", x = -1.2, y = 13, label ="below threshold",size=3,vjust=0.5,hjust=0.5,color="grey30")+
  
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+
  
  annotate("text", x = 13, y = -1, label ="multiple bats",size=3,angle=45, vjust=-1.5,color="grey30")+
  
  geom_rect(aes(xmin=12.5, xmax=13.5, ymin=0.5, ymax=12.5),fill="NA",
            color="black")+ #vertical box
  geom_rect(aes(xmin=0.5, xmax=12.5, ymin=12.5, ymax=13.5),fill="NA",
            color="black")+ #horizontal box
  theme(plot.title = element_text(hjust = 0.5))+
  theme(axis.title.x = element_text(hjust=1,vjust = 30))

ggsave(paste0("./real_life_data/Batzbach_70_cm_belowth_multiple_corrected.png"),
       width =16, height =14, dpi = 800, units = "cm", device='png')   
```


```{r percentile_differences}
alg=read.csv("./real_life_data/phenology/batz_alg.csv")
hum=read.csv("./real_life_data/phenology/batz_hum.csv")
head(alg)
head(hum)
alg$type="alg"
hum$type="hum"
