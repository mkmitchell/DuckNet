...
```{r human_pheno_plot}
df2=read.csv("./untrained_data/human/Comthurey_pseudo.csv") #human ID with pseudodays
head(df2)
...
Species_list=c("Mnat","Mdau","Mmyo")

  for (Species in Species_list)   {
    
    Species_plot=ifelse(Species=="Mnat","Myotis nattereri",
                        ifelse(Species=="Mdau","Myotis daubentonii","Myotis myotis"))
   ...
   df2 %>% 
    ...
    ggplot(aes(x=pseudodate, y=n))+
      ...
      scale_x_date(breaks = monthly, date_labels= "%b %d", 
                         limits=c((as.Date("2020-08-01")),as.Date("2020-12-31")))+
      theme(axis.text.x = element_text(angle=45, hjust = 1))+
       labs(title=bquote('Comthurey -'~italic(.(Species_plot))),
                      x="", 
      ...
      theme_bw()+
      geom_label(x = as.Date("2020-12-15"), y = max*0.8, 
            aes(label = paste0("N=",scales::comma(N))),  
            inherit.aes = FALSE, fill="white")+
     theme(plot.title = element_text(hjust = 0.5))+
    scale_y_continuous(breaks=pretty_breaks(n=6))
   
   ggsave(paste0("./real_life_data/phenology/Comthurey_human_",Species,".png"),
       width =15, height =7, dpi = 800, units = "cm", device='png')
  }


write.csv(percentiles_hum[-1,], paste0("./real_life_data/phenology/cmt_hum.csv"), 
             row.names = F,
             quote = F)
...
```{r AI_pheno_plot}
df2=read.csv("./untrained_data/AI/Comthurey_pseudo.csv") #AI ID with pseudodays
head(df2)
...
Species_list=c("Mnat","Mdau","Mmyo")
  ...
  for (cSpecies in Species_list)   {
    ...
    Species_plot=ifelse(cSpecies=="Mnat","Myotis nattereri",
                        ifelse(cSpecies=="Mdau","Myotis daubentonii","Myotis myotis"))
   ...
   df2 %>% 
    ...
    scale_y_continuous(breaks=pretty_breaks(n=6))
      ...
      ggsave(paste0("./real_life_data/phenology/Comthurey_th70_",cSpecies,".png"),
       width =15, height =7, dpi = 800, units = "cm", device='png')
  }


write.csv(percentiles_alg[-1,], paste0("./real_life_data/phenology/cmt_alg.csv"), 
             row.names = F,
             quote = F)
...
for(cSpec in Species_list){
  ...
  Species_plot=ifelse(cSpec=="Mnat","Myotis nattereri",
                        ifelse(cSpec=="Mdau","Myotis daubentonii","Myotis myotis"))
...
mdf %>% 
 ggplot(aes(x=n.x, y=n.y))+
  geom_point()+
  geom_abline()+
  labs(x="human",y="BatNet")

CCC=round(epi.ccc(mdf$n.x,mdf$n.y)$rho.c[1],3)
maximum=max(mdf$n.x,mdf$n.y)

mdf$species_label=ifelse(cSpec=="Mnat","Myotis nattereri",
                        ifelse(cSpec=="Mdau","Myotis daubentonii","Myotis myotis"))
...
mdf %>% 
  ...
  ggsave(paste0("./real_life_data/concordance/Comthurey_",cSpec,".png"),
...
```{r mismatch}
full=read.csv("./real_life_data/mismatch_checking/Comthurey_r500_corrected.csv")
...
levels(above_th$pred)=c("Barbastella barbastellus","Empty","Myotis daubentonii","Myotis myotis","Myotis nattereri","Plecotus sp.")
...
ggsave(paste0("./real_life_data/Comthurey_70_cm.png"),
...
cm2 %>% 
  ...
  labs(x="human ID", y="BatNet ID",
       title="Comthurey")+
  annotate(geom="rect", xmin=0.5, xmax=1.5, ymin=6.5, ymax=7.5,fill="#F5FFFF",alpha=0.6)+ #2
  annotate(geom="rect",xmin=1.5, xmax=2.5, ymin=6.5, ymax=7.5,fill="lightblue2",alpha=0.6)+ #61
  annotate(geom="rect",xmin=2.5, xmax=3.5, ymin=6.5, ymax=7.5,fill="lightblue1",alpha=0.6)+ #27
  annotate(geom="rect",xmin=3.5, xmax=4.5, ymin=6.5, ymax=7.5,fill="#C1BBDD",alpha=0.6)+ #486
  annotate(geom="rect",xmin=4.5, xmax=5.5, ymin=6.5, ymax=7.5,fill="#F5FFFF",alpha=0.6)+ #2
  annotate(geom="rect",xmin=5.5, xmax=6.5, ymin=6.5, ymax=7.5,fill="#F5FFFF",alpha=0.6)+ #2
  
  annotate(geom="rect",xmin=6.5, xmax=7.5, ymin=0.5, ymax=1.5,fill="#F5FFFF",alpha=0.6)+ #6
  annotate(geom="rect",xmin=6.5, xmax=7.5, ymin=1.5, ymax=2.5,fill="lightblue1",alpha=0.6)+ #40
  annotate(geom="rect",xmin=6.5, xmax=7.5, ymin=2.5, ymax=3.5,fill="lightblue1",alpha=0.6)+ #20
  annotate(geom="rect",xmin=6.5, xmax=7.5, ymin=3.5, ymax=4.5,fill="#C1BBDD",alpha=0.6)+ #650
  annotate(geom="rect",xmin=6.5, xmax=7.5, ymin=4.5, ymax=5.5,fill="#F5FFFF",alpha=0.6)+ #1
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+
          annotate("text", x = -0.6, y = 7, label ="below threshold",size=3,vjust=0,hjust=0.5,color="grey30")+
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+
          annotate("text", x = 7, y = -0.3, label ="multiple bats",size=3,angle=45, vjust=-0.1,color="grey30")+
  geom_rect(aes(xmin=6.5, xmax=7.5, ymin=0.5, ymax=6.5),fill="NA",
            color="black")+
  geom_rect(aes(xmin=0.5, xmax=6.5, ymin=6.5, ymax=7.5),fill="NA",
            color="black")+
   theme(plot.title = element_text(hjust = 0.5))+
  theme(axis.title.x = element_text(hjust=1,vjust = 30))

ggsave(paste0("./real_life_data/Comthurey_70_cm_belowth_multiple.png"),
       width =16, height =12, dpi = 800, units = "cm", device='png')   
```

```{r percentile_differences}
alg=read.csv("./real_life_data/phenology/cmt_alg.csv")
hum=read.csv("./real_life_data/phenology/cmt_hum.csv")
head(alg)
head(hum)
alg$type="alg"
hum$type="hum"
